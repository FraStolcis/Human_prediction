...
class DataPreparation():
    ...
    def dist_calc(self,distance,j,n_steps_out,obj,obj_name):
        ...
        for _ in range(n_steps_out):
            if obj_name == 'obj1':
                ...
                distance = append(distance, distance[-1]) 
        plt.plot(distance)
        plt.show()           
    ...
    def data_preparation_multistep_dist(self,batch_size,interest_feature,n_steps_in,n_steps_out,target_feature,obj,obj_name):   
        #INPUT: quanti file, lista di feature di interesse, numero di step in input, numero di step in output  
        #OUTPUT: X e y in formato buono per il modello, numero di step in input, numero di feature di interesse
        ...
        for obj in obj:
            ...
            for i in range(batch_size):
                distance = []
                distance = self.dist_calc(distance,i,n_steps_out,obj,obj_name)
...
def grabstatus(self,distance,obj_name):
    status = []
    if obj_name == 'obj1':
